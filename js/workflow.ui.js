...
WorkFlow_UI.toolbox =
{
  ...
  renderWorkFlowForm : function()
  {
    ...
    group.append('<label class="togvis control-label" style="width: 0; text-transform:capitalize" for="mainWFName">Workflow name:</label>');
  ...
  displayObject :function(obj)
  {
    ...
    _.each(toDisplay, function(val, key)
    {
      ...
      if(key == "inputs" || key ==  "outputs")
      {
        ...
        control = $('<div class="controls" style="display:none; margin: 5px;"></div>');
      }
      else
      {
        group.append('<label class="control-label" style="width: 0; text-transform:capitalize" for="' + key + '">' + key + '</label>');
        control = $('<div class="controls" style="margin: 5px;"></div>');
...
WorkFlow_UI.io =
{
    ...
    displayIO : function()
    {
      ...
      select = $(' <select id="inputs_' + noRows + '" onchange="WorkFlow_UI.io.displayInputDetails(' + noRows + ',this);"></select>');
    ...
    setIO : function(row)
    {
      ...
      select = $('#outputs_' + row).find(":selected");
...
WorkFlow_UI.ioWorkFlow =
{
    ...
    setIO : function(row)
    {
      ...
      if(this.getStartEndComponent().type == "start")
      {
        resultCon = this.getStartEndComponent().obj.parent.addInput(newIO);
      }
      else
      {
        resultCon = this.getStartEndComponent().obj.parent.addOutput(newIO);
      }
    ...
    deleteIO : function(row)
    {
      ...
      if(this.getStartEndComponent().type == "start")
      {
        resultCon = this.getStartEndComponent().obj.parent.deleteInput(newIO);
      }
      else
      {
        resultCon = this.getStartEndComponent().obj.parent.deleteOutput(newIO);
      }
